...
```
<repo root>/
├── map.html                  ← THE ENTIRE FRONT-END APP (HTML+CSS+JS, ~8,300 lines)
...
├── scripts/
│   ├── fetch-inat-data.js    ← THE DATA PIPELINE (Node ESM, ~1,600 lines)
...
const CLIM_CACHE_VER = 3;        // bump to force a full climate re-fetch
```
...
3. **Climate:** fetch NASA POWER daily `T2M_MAX/T2M_MIN/PRECTOTCORR/RH2M` for each
   cell (cached in `climate-cache.json` keyed by cell — the expensive step).
...
5. **Aggregate per species** (split N/S hemisphere so seasons align):
   ...
   - `monthly_nh/_sh` = per-month [p25,p50,p75] envelope + daily-extreme band
...
One file. Loads Leaflet + Chart.js + Supercluster from CDNs. Structure (top→bottom):
...
*Last updated: 2026-05 (post: humidity-aware lapse, daily-extreme tiers, phenology,
